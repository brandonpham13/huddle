...
export function initUserRoutes(app: Express) {
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      const { sleeperUsername } = req.body as {
      ...
      if (username) {
        ...
        if (!sleeperProvider) {
          res.status(503).json({ error: "Sleeper provider unavailable" });
          return;
        }
        try {
      ...
      try {
        await clerkClient.users.updateUserMetadata(userId, {
          unsafeMetadata: {
            ...
            sleeperUserId,
            // Clear synced leagues when unlinking so re-linking starts fresh
            ...(isUnlink ? { syncedLeagueIds: [] } : {}),
          },
        });
      } catch {
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      ...
      if (
        !Array.isArray(syncedLeagueIds) ||
        !syncedLeagueIds.every((id) => typeof id === "string")
      ) {
        res
          .status(400)
          .json({ error: "syncedLeagueIds must be an array of strings" });
        return;
      }
