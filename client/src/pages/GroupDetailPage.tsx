  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="p-6 max-w-3xl mx-auto space-y-4">
        ...
        {groupQuery.isError && (
          <Card><CardContent className="py-8 text-center text-red-500">Failed to load group.</CardContent></Card>
        )}
...
function rosterTeamName(roster: Roster, leagueUsers: TeamUser[]): string {
  const owner = roster.ownerId ? leagueUsers.find(u => u.userId === roster.ownerId) : null
...
function RosterTable({
  ...
  const sorted = useMemo(() => [...rosters].sort((a, b) => a.rosterId - b.rosterId), [rosters])

  return (
    <Card>
      <CardHeader>
        <CardTitle>Teams</CardTitle>
        <CardDescription>Claim status for every roster in this group.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-2">
          {sorted.map(roster => {
            ...
            return (
              <div key={roster.rosterId} className="flex items-center justify-between py-2 border-b last:border-b-0">
                <div className="flex items-center gap-3 min-w-0">
                  <span className="text-xs text-gray-400 w-5">{roster.rosterId}</span>
                  <span className="text-sm font-medium truncate">{teamName}</span>
                </div>
                <div className="text-xs text-gray-500">
                  {claim
                    ? <>Claimed by <span className="font-medium text-gray-900">{describeUser(claim.user)}</span></>
                    : <span className="text-gray-400">Unclaimed</span>}
                </div>
              </div>
            )
          })}
          {sorted.length === 0 && <p className="text-sm text-gray-500">No rosters loaded yet.</p>}
        </div>
      </CardContent>
    </Card>
  )
...
function ClaimTeamCard({
  ...
  leagueUsers: TeamUser[]
  claims: GroupClaimSummary[]
  myClaim: { id: string; rosterId: number; status: 'pending' | 'approved' | 'rejected' } | null
}) {
  ...
  const approvedRosterIds = useMemo(
    () => new Set(claims.filter(c => c.status === 'approved').map(c => c.rosterId)),
    [claims],
  )
  const availableRosters = useMemo(
    () => rosters.filter(r => !approvedRosterIds.has(r.rosterId)).sort((a, b) => a.rosterId - b.rosterId),
    ...
    return (
      <Card>
        <CardContent className="py-4 text-sm text-amber-700">
          Your claim for roster #{myClaim.rosterId} is pending commissioner approval.
        </CardContent>
      </Card>
    )
  }

  const canSubmit = rosterId !== null && password.length >= 4 && !submit.isPending

  return (
    <Card>
      <CardHeader>
        <CardTitle>Claim a team</CardTitle>
        <CardDescription>Enter the group password to request your team.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-3">
          ...
          <div>
            <label className="text-xs text-gray-500 block mb-1">Group password</label>
            <input
              type="password"
              value={password}
          ...
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              onClick={() => {
                ...
                submit.mutate(
                  { groupId, rosterId, password, message: message.trim() || undefined },
                  {
                    onSuccess: () => {
                      setPassword('')
                      setMessage('')
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {pending.length === 0 ? 'No pending requests.' : `${pending.length} waiting for review.`}
        </CardDescription>
      </CardHeader>
      <CardContent>
        {pending.length > 0 && (
              ...
              <div key={claim.id} className="border rounded-md p-3 bg-gray-50">
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <p className="text-sm font-medium">{describeUser(claim.user)}</p>
                    <p className="text-xs text-gray-500">
                      requesting <span className="font-medium text-gray-700">{teamNameForRoster(claim.rosterId)}</span>
                      {claim.user?.email && claim.user.username ? ` · ${claim.user.email}` : ''}
                    </p>
                    {claim.message && (
                      <p className="text-xs text-gray-500 mt-1 italic">"{claim.message}"</p>
                    )}
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => decide.mutate({ groupId, claimId: claim.id, decision: 'rejected' })}
                    ...
                    <Button
                      size="sm"
                      onClick={() => decide.mutate({ groupId, claimId: claim.id, decision: 'approved' })}
                      disabled={decide.isPending}
                    >
                      Approve
                    </Button>
        ...
        {decide.isError && (
          <p className="text-xs text-red-500 mt-2">{(decide.error as Error).message}</p>
        )}
      </CardContent>
    </Card>
  )
