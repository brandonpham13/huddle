...
export function LeaguesPage() {
  ...
  const availableYears = leagues
    ? [...new Set(leagues.map((l) => l.season))].sort(
  ...
  const visibleLeagues =
    leagues?.filter((l) => l.season === selectedYear) ?? [];

  // Family root map: detect leagues already imported under a different year's ID
  const familyRootMap = leagues
    ? buildFamilyRootMap(leagues)
    : new Map<string, string>();
  const syncedRoots = new Set(
    pendingIds.map((id) => familyRootMap.get(id) ?? id),
  );
  ...
  const hasChanges =
    ...
    JSON.stringify([...syncedLeagueIds].sort());
          ...
          <>
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Select leagues to sync to your dashboard. Synced leagues
                  appear in your league switcher.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {/* Year selector */}
                ...
                {isError && (
                  <p className="text-sm text-red-500">
                    Failed to load leagues. Check that your Sleeper username is
                    correct.
                  </p>
                )}
                ...
                {!isLoading && visibleLeagues.length === 0 && (
                  <p className="text-sm text-gray-500">
                    No leagues found for {sleeperUsername} in {selectedYear}.
                  </p>
                )}
                ...
                {visibleLeagues.length > 0 && (
                  <div className="space-y-2">
                    {visibleLeagues.map((league) => {
                      ...
                      const isImportedElsewhere =
                        !isSynced && syncedRoots.has(root);
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-2 mt-0.5">
                                <span
                                  className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${leagueStatusBadge(league.status)}`}
                                >
            ...
            {importedFamilies.length > 0 && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    These leagues are synced to your dashboard.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
                    {importedFamilies.map(
                      ({ representative: league, allIds }) => (
                        <div
                          key={allIds.join(",")}
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-gray-400 mt-0.5">
                                  {allIds.length > 1
                                    ? `${allIds.length} seasons imported`
                                    : league.season}
                                </p>
                              </div>
                            </div>
